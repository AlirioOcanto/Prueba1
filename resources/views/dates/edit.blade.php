        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class=" overflow-hidden shadow-xl sm:rounded-lg p-3">
                <div class="flex items-center justify-center">
                    <h2 class="text-3xl font-semibold">Agendar una instalación de Iluminación a un Usuario</h2>
                </div>
                <div class="my-5">
                    <div class="relative overflow-x-auto ">
                        <form action="{{ route('date.illumination.update', $calendar) }}" method="POST" >
                            @method('PUT')
                            @csrf
                            <div class="grid grid-cols-2 gap-4">
                                ...
                                <div class="mb-4">
                                    ...
                                    <input type="text" value="{{old('user_name', $calendar->user_name)}}" name="user_name" id="user_name" class="form-input rounded-lg w-full" />
                                    {{-- show error --}}
                                    @error('user_name')
                                ...
                                {{-- illuminations_id --}}
                                 <div class="mb-4">
                                    ...
                                    <select name="illuminations_id" id="brand" class="form-input rounded-md shadow-sm mt-1 block w-full">
                                        <option value="">Seleccione una marca</option>
                                        @foreach ($products as $brand)
                                            <option value="{{ $brand->id }}"  @selected($brand->name === $calendar->illuminations->name)    >{{ $brand->name }} </option>
                                        @endforeach
                                    </select>
                                    {{-- show error --}}
                                    @error('illuminations_id')
                                        <span class="text-red-500">{{ $message  }}</span>
                                    @enderror
                                </div>
