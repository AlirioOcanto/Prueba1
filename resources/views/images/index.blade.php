        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class=" overflow-hidden shadow-xl sm:rounded-lg p-3">
                <div class="flex items-center justify-between">
                    ...
                    <h2 class="text-3xl">Agregar image a {{ $sale->brand->name }} - {{ $sale->model }}</h2>
                    {{-- <div class="">
                ...
                <form class="my-5" action="{{ route('sales.images.upload', $sale) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('POST')
                    ...
                    @if ($sale->images->count() > 0)
                        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                            @foreach ($sale->images as $image)
                                <div class="relative">
                                    <img src="{{ asset('storage/' . $image->path) }}" alt="{{ $sale->brand->name }} - {{ $sale->model }}" class="w-full h-40 object-cover rounded-lg">
                                     <div class="flex justify-center">
                                        <form action="{{ route('sales.images.destroy',  ['sale' => $sale, 'image' => $image]) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="px-2 py-2 text-sm mt-3 rounded-lg bg-red-500 text-white font-semibold hover:bg-red-700 transition ease flex items-center gap-2">
                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
