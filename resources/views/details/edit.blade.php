        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class=" overflow-hidden shadow-xl sm:rounded-lg p-3">
                <div class="flex items-center justify-center">
                    <h2 class="text-3xl font-semibold">Editar Instalación para: <span class="text-4xl text-blue-500">{{$illumination->name}} - {{ $illumination->model }}</span> </h2>
                </div>
                <div class="my-5">
                    <div class="relative overflow-x-auto ">
                        <form action="{{ route('lightning.instalation.update', $illumination) }}" method="POST" >
                            @method('PUT')
                            @csrf
                            <div class="grid grid-cols-2 gap-4">
                                <div class="mb-4 col-span-2">
                                    <label for="" class="block text-gray-700 text-sm font-bold mb-2">Descripción(*Campo Opcional):</label>
                                    <textarea name="description"  id="editor" cols="30" rows="10" class="p-4">{{ old("description", $illumination->description) }}</textarea>
                                    @error('description')
